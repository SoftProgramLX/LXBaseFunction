...
@implementation LXAudioToolboxPlaySound
...
{
    NSString *path;
    if (type == SoundTypeSystem) {
        path = [NSString stringWithFormat:@"/System/Library/Audio/UISounds/%@", soundName];
    } else {
        path = [[NSBundle mainBundle] pathForResource:soundName ofType:nil];
        NSAssert(path, @"没找到播放文件!");
    }
    
    NSURL *audioFile = [NSURL fileURLWithPath:path];
    SystemSoundID soundID = 0;
    /**
     * audioFile:音频文件url
     * soundID:声音id（此函数会将音效文件加入到系统音频服务中并返回一个长整形ID）
     */
    OSStatus error = AudioServicesCreateSystemSoundID((__bridge CFURLRef)(audioFile), &soundID);
...
/**
 ...
 ID     (iPhone)        (iPod Touch)	Category	Note
