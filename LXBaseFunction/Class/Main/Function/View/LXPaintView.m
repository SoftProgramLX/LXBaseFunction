...
#import "LXPaintStep.h"

@interface LXPaintView ()
{
...
-(void)createColorBord{
    ...
    //色板的view
    UIView *colorBoardView = [[UIView alloc]initWithFrame:CGRectMake(0, 20, kScreenWidth, 20)];
    ...
    for (int i =0; i<colors.count; i++) {
        UIButton *btn = [[UIButton alloc]initWithFrame:CGRectMake((kScreenWidth/colors.count)*i, 0, kScreenWidth/colors.count, 20)];
...
-(void)createStrokeWidthSlider{
    slider = [[UISlider alloc]initWithFrame:CGRectMake(0, 50, kScreenWidth, 20)];
...
-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    LXPaintStep *paintStep = [[LXPaintStep alloc]init];
...
-(void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    //获取当前路径
    LXPaintStep *step = [paintSteps lastObject];
    NSMutableArray *pathPoints = step->pathPoints;
    //获取当前点
